function [x_min_m, f_min_m, f_vals, runtime] = WingsuitFlyingSearch(fun, n, constraints, N, M)
    % --------------------------------------------------------------------------
    % Wingsuit Flying Search algorithm
    % Inputs:
        % 'fun' - function needs to be minimized
        % 'n' - dimensionality of the search space
        % 'constraints' - constraints of the search space
    ...
    for i = 1:n
        x(:,i) = x(:,i).*(constraints(i,2)-constraints(i,1))+constraints(i,1);
    end        
    
    f = GetValues(x, n, constraints, fun);	% values of the initial points
    ...
    f_temp = GetValues([x_centr; x_rand], n, constraints, fun); 
    x = [x; x_centr; x_rand];
    f = [f; f_temp];
    ...
    % Plotting results
    if plot_res     
        num_x = PlotRes(x, f, n, num_x, constraints, f_min_m);
    end
    ...
    while(SC == false)
        ...
        f_temp = GetValues(x(x_size+1:size(x,1),:), n, constraints, fun);   % new points values
